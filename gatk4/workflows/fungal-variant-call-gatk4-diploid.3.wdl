# Variant calling pipeline for fungal diploid genomes
# Developed by Aina Martinez Zurita (amartine@broadinstitute.org), Xiao Li (xiaoli@broadinstitute.org), Christina Cuomo (cuomo@broadinstitute.org)
...
###diploids: updated HaplotypeCaller ploidy to 2
  command {
    ${gatk_path} --java-options "-Xmx${mem_size_gb}G" HaplotypeCaller \
    -R ${ref} -I ${input_bam} -O ${gvcf_name} -ERC GVCF -ploidy 2
  }
...
###diploids: hard filtering updated per https://gatk.broadinstitute.org/hc/en-us/articles/360035890471-Hard-filtering-germline-short-variants and https://gatk.broadinstitute.org/hc/en-us/articles/360035531112--How-to-Filter-variants-either-with-VQSR-or-by-hard-filtering
  command {
    #Select variants, both SNPs and raw_indels. Mixed sites are discarded.
    ...
    -filter "MQRankSum < -12.5" --filter-name "MQRS12.5" \
    -filter "ReadPosRankSum < -8.0" --filter-name "RPRS8" \
...
### for diploids, removed AD filter (--min_percent_alt_in_AD 0.8) and changed output file name lines 578 and 582
  command {
    python /broad-fungalgroup/scripts/SNPs/filterGatkGenotypes.py --min_GQ 50 \
    --min_total_DP 5 ${vcf_file} \
     > ${output_filename}.filtered_SNPs_GQ50_DP10.vcf 2> ${output_filename}.variant_qc_genotype_filter.tsv
  }

  output {
    File vcf_filtered= "${output_filename}.filtered_SNPs_GQ50_DP10.vcf"
